(function($){
	...
	for (var i = 0; i < playlist.length; i++){
		var item = playlist[i];
		$('#playlist').append('<li class="lib" style="overflow:hidden;"><span style="margin-left: 5px;">'+item.title+'</span><span style="float: right;" class="artist">'+item.artist+'</span></li>');
	}
	...
	var play = function(){
		audio.play();
		if (isRotate) {
			$("#pContent .ssBtn").css("animation","9.8s linear 0s normal none infinite rotate");
		    $("#pContent .ssBtn").css("animation-play-state","running");
	    }
	...
	var pause = function(){
		audio.pause();
		$("#pContent .ssBtn").css("animation-play-state","paused");
